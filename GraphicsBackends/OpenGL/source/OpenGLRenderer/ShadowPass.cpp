...
namespace chai::brew
{
        ...
        for (int i = 0; i < 16; i++) {
            ...
            for (int c = 0; c < NUM_CASCADES; c++) {
                ...
                glFramebufferTexture2D(
                    GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, cascade.shadowTex, 0);
    ...
    std::vector<Vec3> ShadowPass::getFrustumCornersWorldSpace(const Mat4& viewProj)
    {
        ...
        for (int x = 0; x < 2; ++x) {
            for (int y = 0; y < 2; ++y) {
                for (int z = 0; z < 2; ++z) {
                    Vec4 pt =
                        invViewProj * Vec4(2.0f * x - 1.0f, 2.0f * y - 1.0f, 2.0f * z - 1.0f, 1.0f);
    ...
    Mat4 ShadowPass::calculateLightViewProjForCascade(const Vec3& lightDir,
                                                      ...
                                                      float farSplit)
    {
        ...
        Vec4 shadowOrigin = lightView * Vec4(center.x, center.y, center.z, 1.0f);
        shadowOrigin = shadowOrigin * texelsPerUnit;
        Vec4 roundedOrigin = Vec4(std::round(shadowOrigin.x),
    ...
    void ShadowPass::execute(void* backend,
                             ...
                             float farPlane)
    {
        ...
        for (size_t i = 0; i < lights.size(); ++i) {
            ...
            if (lightType != 0.0f)
                continue; // CSM only for directional lights

            Vec3 lightDir = normalize(Vec3(light.light.directionAndRange.x,
            ...
            for (int c = 0; c < NUM_CASCADES; c++) {
                ...
                auto* ub =
                    openGLBackend->getUniformManager().getUniformBufferData(*lightData.lightProj);
                if (ub) {
